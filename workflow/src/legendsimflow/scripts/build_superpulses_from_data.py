...
def main():
    ...
    try:
        ...
        period = parts[1]
        run = parts[2]
        run_type = parts[3]  # 'ssc', 'phy', or 'cal'
    except IndexError as err:
        msg = (
            f"runid '{args.runid}' does not match expected format l200-period-run-type"
        )
        raise ValueError(msg) from err

    # Data Files
    data_path = Path(
        "/global/cfs/cdirs/m2676/data/lngs/l200/public/prodenv/prod-blind/auto/latest"
    )
    raw_base = data_path / "../../ref/v3.0.0/generated/tier/raw/ssc" / period / run
    dsp_base = data_path / "generated/tier/dsp/ssc" / period / run
    evt_base = data_path / "generated/tier/evt/ssc" / period / run

    raw_files = sorted(raw_base.glob("*.lh5"))[:10]
    dsp_files = sorted(dsp_base.glob("*.lh5"))[:10]
    evt_files = sorted(evt_base.glob("*.lh5"))[:10]

    if not evt_files:
        msg = f"No EVT files found matching ssc/{period}/{run}/*.lh5"
    ...
    _, dsp_config, _, _ = lookup_currmod_fit_inputs(
        l200data=data_path, metadata=lmeta, runid=args.runid, hpge=args.detector
    )
    logger.info("Found DSP config: %s", dsp_config)

    # Slices to process: fixed energy 1500-2000 keV, drift time 500-2500 ns in 200 ns steps
    slices = [
        Slice(
