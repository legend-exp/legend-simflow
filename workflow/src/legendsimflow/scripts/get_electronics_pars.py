...
def main():
    ...
    parser.add_argument(
        "--output-file", type=str, required=True, help="Path to output YAML file"
    )
    ...
    if not data_superpulses:
        msg = f"no superpulses found in drift time range [{DT_RANGE_TUNING[0]:.0f}, {DT_RANGE_TUNING[1]:.0f}] ns"
        raise RuntimeError(msg)
    logger.info(
        "selected %d slices in [%.0f, %.0f] ns", len(data_superpulses), *DT_RANGE_TUNING
    )

    comparison_window = (
        tuple(args.comparison_window) if args.comparison_window else None
    )

    # Prepare ideal waveforms
    logger.info("selecting ideal waveforms per slice ...")
    ideal_wfs = get_ideal_wfs_all_slices(ideal_lib, data_superpulses, angle=args.angle)

    # Run fit
    logger.info(
        "starting fit (sigma0=%.1f, tau0=%.1f) ...", args.sigma_start, args.tau_start
    )
