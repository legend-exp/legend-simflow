...
def create_response_kernel(
    dt, mu_bandwidth, sigma_bandwidth, tau_rc, gaussian_only=False
):
    """
...
def apply_mwa(wf_array):
    ...
    mwa_currents = np.zeros_like(
        wf_array, dtype=float
    )  # Risk to saturate memory?? Should check this??
...
def shift_array(
    wf_input, center_idx=ALIGNMENT_IDX, output_len=NSAMPLES_OUTPUT_CURRENT_WFS
):
    """
...
def generate_realistic_map(
    ideal_wf_map_obj, rf_kernel, Amax_alignment_idx=ALIGNMENT_IDX
):
    """
...
def write_realistic_struct_to_lh5(
    realistic_dict, original_struct, output_filename, group_name
):
    """
...
def main():
    ...
    rf_kernel = create_response_kernel(
