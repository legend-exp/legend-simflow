...
def metadata_dirnames(config: SimflowConfig) -> list[Path]:
    ...
    The list starts with the `legend-metadata` clone. It ends with the fallback
    metadata of the experiment, if there is one. This is the order
    ...
    fallback = metautils.fallback_metadata_dirname(config)
    if fallback.is_dir():
        dirnames.append(fallback)
...
def output_elecmod_filename(config: SimflowConfig, **kwargs) -> Path:
    """The path to the per-detector HPGe electronics pulse model parameter file."""
    return _expand(
        config.paths.pars / "hpge/elecmod/{runid}-{hpge_detector}-model.yaml",
        **kwargs,
    )


def output_elecmod_merged_filename(config: SimflowConfig, **kwargs) -> Path:
