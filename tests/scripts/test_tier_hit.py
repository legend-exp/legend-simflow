...
def test_hit_script_cli(
    ...
    crystal_impurity_status_vals = _field(first_det, "crystal_impurity_status")
    assert np.all(np.isin(crystal_impurity_status_vals, [0, 1, 2])), (
        f"invalid crystal_impurity_status codes: {np.unique(crystal_impurity_status_vals)}"
    )
