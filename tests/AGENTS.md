...
- `l1000dsg01`: used by `test_l1000_workflow`, which exercises the vtx→pdf
  ...
  and detectors come from the fallback metadata in
  `inputs/simprod/config/metadata/l1000dsg01/`, and not from the
  `legend-metadata` stand-in in `inputs/`. The fallback metadata uses period
  `p99` and year-2000 start keys, so every lookup misses in `inputs/` and
  reaches the fallback (see `legendsimflow.metadata.lookup`). `V02160A` and
  `V05261B` are in both databases and resolve to `inputs/`. `V99900A` is in the
  fallback metadata alone. The fallback metadata is small and hand-written, so
  its detectors do not match the GDML geometry, which `legend-pygeom-l1000`
  builds from its own packaged configuration. A real production commits the tree
  that `legend-pygeom-l1000 --write-metadata` produces
