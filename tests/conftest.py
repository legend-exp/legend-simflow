...
@pytest.fixture(scope="session")
def l1000_config():
    """The dummy LEGEND-1000 production, which runs on fallback metadata.

    Built through :func:`legendsimflow.utils.init_simflow_context`, unlike
    :func:`make_config`, so that the fallback metadata is attached the same way
    the Snakefiles get it.
    """
