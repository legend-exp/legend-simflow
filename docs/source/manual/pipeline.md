...
1. `simulate_electron_gun` (one job by default, see
   {ref}`aoemeanmod-settings-meta`) builds the production geometry from the
   experiment's template geometry configuration, renders the _remage_ macro from
   the configured template (GPS mono-energetic electrons, confined in the bulk
   of all germanium sensitive volumes of the geometry) and runs _remage_ through
   its Python API once per electron energy (500 to 3000 keV in 500 keV steps,
   about one second per thousand electrons after initialisation). The `stp`
   output files (one per energy) are temporary: they are deleted once consumed
   by all the extraction jobs. A plot of the primary vertices checks the
   confinement.
2. `extract_hpge_aoemean_energy_dependence` (one job per `runid`, since all the
   ...
   correction. Electrons are single-site events, so the raw A/E at each energy
   is summarised by its unbinned mode (the half-sample mode, insensitive to the
   low-side tail left by escaping or re-absorbed bremsstrahlung; its uncertainty
   comes from a bootstrap). The modes are fitted with the same linear model and
   Minuit procedure used on data ({class}`pygama.pargen.AoE_cal.Pol1`), which is
   the model consumed by the `hit` tier. The per-run model file has the same
   format as the `aoemeanmod_default` hit-tier setting; a statistics file stores
   the per-energy modes and widths, and a validation PDF shows the A/E
   distributions at each energy together with the fit, for every detector.
