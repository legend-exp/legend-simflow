...
- `custom_G4NDL`: Optional. Run this simulation against a custom G4NDL
  neutron-data library generated by
  [custom-g4ndl-generator](https://github.com/legend-exp/custom-g4ndl-generator).
  Either a string (the library `source`, e.g. `JEFF-3.3`) or a mapping with a
  `source` plus generator options. The option keys map directly to the
  `custom-g4ndl` CLI flags. Only that simulation's _remage_ call is wrapped in
  an `env` setting `G4NEUTRONHPDATA`/`G4PARTICLEHPDATA`.
