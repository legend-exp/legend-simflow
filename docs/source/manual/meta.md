...
- `simulate_psd_with_psl` (bool, default `False`): enable the
  ...
  simulation entirely.
- `tune_impurity_curve` (bool, default `True`): select the HPGe modeling
  criteria based on the tuning of the crystal impurity curve to data. When
  `False`, detectors are selected by their operational voltage margin above
...
the built-in defaults are used. It is only read when `tune_impurity_curve` is
`False` in {ref}`hit-tier-settings`.
